...
export default function FiscalDay() {
  ...
  async function fetchStatus() {
    ...
    if (dev) {
      setDevice(dev);
      // Find the open day or the last closed day
  ...
  if (!device) return <div className="p-10">Loading Device Status...</div>;
      ...
      <div className={`rounded-xl p-8 text-center border-2 ${isDayOpen ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'}`}>
        ...
        {isDayOpen ? (
          <button 
            ...
            disabled={loading}
            className="bg-red-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-red-700 shadow-lg disabled:opacity-50 flex items-center justify-center gap-2 mx-auto"
          >
          ...
          <button 
            ...
            disabled={loading} 
            className="bg-indigo-600 text-white px-6 py-3 rounded-lg font-bold hover:bg-indigo-700 shadow-lg disabled:opacity-50 flex items-center justify-center gap-2 mx-auto"
          >
      ...
      <div className="mt-8">
        ...
        <div className="bg-white border rounded-lg divide-y">
          {device.fiscal_days?.map(day => (
            <div key={day.id} className="p-4 flex justify-between items-center">
              <div>
                <div className="font-bold">Day {day.fiscal_day_no}</div>
                <div className="text-xs text-gray-500">
                  {new Date(day.opened_at).toLocaleDateString()} - {day.closed_at ? new Date(day.closed_at).toLocaleTimeString() : 'Active'}
                </div>
              </div>
              <div className="text-right">
                <div className="font-mono font-bold">${day.total_sales_amount || '0.00'}</div>
                {day.status === 'FiscalDayClosed' && (
                  <button className="text-xs text-indigo-600 font-medium flex items-center gap-1 justify-end mt-1">
